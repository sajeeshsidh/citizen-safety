...
interface PoliceHistoryCardProps {
    alert: Alert;
...
const PoliceHistoryCard: React.FC<PoliceHistoryCardProps> = ({ alert, onClick, isSelected, isActive = false, onDelete }) => {
    const isCanceled = alert.status === 'canceled';
    const isMissed = alert.status === 'timed_out';

    const getStatusInfo = () => {
        if (isActive) return { icon: <ShieldCheckIcon width={20} height={20} color="#facc15" />, text: 'ACTIVE', color: '#facc15' };
        ...
        return { icon: <ShieldCheckIcon width={20} height={20} color="#4ade80" />, text: 'RESOLVED', color: '#4ade80' };
    };
    ...
    >
      ...
      <View style={styles.footer}>
          <Text style={[styles.statusText, { color: statusInfo.color }]}>{statusInfo.text}</Text>
            ...
            <Text style={styles.officerText}>By Officer #{alert.acceptedBy}</Text>
          )}
      </View>
    </TouchableOpacity>
  );
...
export default PoliceHistoryCard;
